  ...
  <div class="colors">
    <div class="color-box">
      <div class="prime">
        <p>Primärfarben</p>
        <Color
          v-for="(item, index) in primeColors"
          :key="'item' + index"
          :color="item"
          @update="displayColor"
        />
      ...
      <div class="second">
        <p>Sekundärfarben</p>
        <Color
          v-for="(item, index) in secondColors"
          :key="'item' + index"
          :color="item"
          @update="displayColor"
        />
...
export default {
  ...
  methods: {
    displayColor(variable) {
      ...
      this.rgb = variable.rgb
        .slice(4, variable.rgb.length - 1)
        .replace(/,/g, "");
    ...
    copy(number) {
      var copyText = document
        .getElementById("colordata")
        .getElementsByTagName("input")[number - 1];
      copyText.select();
