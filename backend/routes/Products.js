...
products.get('/get_all_products', async function (req, res) {
  await db.query(
  `WITH image AS (SELECT p_v.p_id, p_v.color, array_agg(i.image) AS images FROM e_product_variants p_v JOIN r_product_images p_i ON p_v.id = p_i.p_id LEFT JOIN e_images i ON p_i.i_id = i.id GROUP BY p_v.p_id, p_v.color) SELECT p.name AS name, p_v.color AS color, p.price AS price, c.name AS category, p.name || p_v.color AS unique_token, i.images AS image, json_agg(json_build_object('size', p_v.size, 'p_v_id', p_v.id, 'amount', p_v.quantity)) AS stock FROM e_products p JOIN e_product_variants p_v ON p.id = p_v.p_id JOIN enum_categories c ON c.id = p.category JOIN image i ON i.p_id = p.id AND i.color = p_v.color GROUP BY  p.name, p_v.color, c.name, p.price, unique_token, image;`,
  { type: QueryTypes.SELECT }).then(function(result) {
    if(result !== null) res.status(200).json(result);
